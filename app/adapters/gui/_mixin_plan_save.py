...
Noten-/Farb-Edit um wenige hundert Millisekunden (statt ihn synchron bei
jedem einzelnen Edit auszuführen), damit eine schnelle Serie solcher Edits
(z. B. eine ganze Notenspalte durchgehen) nur einen Schreibvorgang statt
vieler auslöst. Der Undo/Redo-Verlauf (``PlanHistory.record()``) bleibt davon
unberührt — der bleibt synchron, nur das eigentliche ``save_plan()`` wird
verzögert (siehe ``HandlerContext.plan_save_scheduler`` und
``_shared.py::_record_and_save``).

Nach dem exakt gleichen Muster wie der bestehende Namens-Debounce
...
kurze Verzögerung (Standard 400ms, ``DEFAULT_PLAN_SAVE_DELAY_MS``), nicht
grundsätzlich — dieselbe Race-Bedingung bestand vorher bei jedem synchronen
Save ebenfalls, nur mit einem kürzeren Fenster.
"""
...
from app.adapters.gui.main_window_constants import DEFAULT_PLAN_SAVE_DELAY_MS, LOGGER
...
class PlanSaveMixin:
    ...
    def _schedule_plan_save(self, plan: SeatingPlan, path: Path) -> None:
        ...
        self._plan_save_after_id = self.after(DEFAULT_PLAN_SAVE_DELAY_MS, self._flush_pending_plan_save)
