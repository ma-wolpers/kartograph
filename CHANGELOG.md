...
### Added
- Neue "Symbol-Verwaltung" (Ansicht-Menü und Editor-Toolbar): zeigt alle Symbole — eingebaute mit ihren Bewertungsstufen-Erklärungen und eigene, pro Sitzplan definierte Doku-Symbole — in einer Liste. Über "Neu" lassen sich eigene Doku-Symbole anlegen (ein beliebiges Zeichen/Emoji einfügen, eine Bedeutung eintragen, ein Tastenkürzel als einzelner Buchstabe vergeben, z. B. `L`), über "Bearbeiten"/"Löschen" wieder ändern bzw. entfernen (Symbol/Kürzel sind änderbar, die Bedeutung nicht — für eine Umbenennung: löschen und neu anlegen). Eigene Symbole toggeln wie "Abwesend"/"Nicht abgegeben" tagesbezogen binär (Ein/Aus) in der Dokutabelle (per Maus oder dem vergebenen Tastenkürzel) und erscheinen dadurch am aktuellen Tag auch temporär auf dem Tisch im Sitzplan-Raster. Wird ein eigenes Symbol gelöscht, bleiben bereits erfasste Dokumentationstage erhalten (sie zeigen es danach als "Gelöschtes Symbol" an).
- Neues Doku-Symbol "Aufgaben nicht gemacht" (`☐`, Tastenkürzel `t`) — funktioniert wie "Abwesend"/"Nicht abgegeben": tagesbezogen in der Dokutabelle setzbar, für den aktuellen Tag auch temporär auf dem Tisch im Sitzplan-Raster sichtbar.
- Neue Export-Funktion "Sitzkärtchen exportieren (ZIP)" (Datei-Menü und Editor-Toolbar): erzeugt für jeden benannten Schüler eine kleine, transparente PNG-Miniaturkarte des Sitzplans — alle Tische weiß mit schwarzer Umrandung, der Lehrertisch orange, der eigene Tisch des jeweiligen Schülers kräftig blau, ohne Namen, Noten, Symbole oder Legende. Alle Kärtchen landen in einem einzigen ZIP-Archiv, benannt nach dem effektiven Vornamen (Spitzname überschreibt wie überall sonst auch den offiziellen Vornamen), bei Namensvettern ergänzt um so viel Nachname wie zur Unterscheidung nötig (z. B. "Emil.png", "Gustav Ma.png", "Gustav Mu.png").
- Sitzpläne können jetzt archiviert werden, statt sie löschen zu müssen: ein neuer Knopf über der Planliste archiviert den ausgewählten Sitzplan (mit Bestätigung) bzw. stellt ihn wieder her, falls er bereits archiviert ist (ohne erneute Nachfrage) — derselbe Knopf passt sich also dem Auswahlzustand an. Archivierte Pläne verschwinden standardmäßig aus der Planliste; über "Ansicht → Archivierte Sitzpläne anzeigen" lassen sie sich ein- und ausblenden (Einstellung bleibt über einen Neustart hinweg erhalten) und erscheinen dort in Klammern, z. B. "(Klasse 5a)", zur klaren Unterscheidung von normalen Plänen. Ein archivierter Plan bleibt ganz normal per Doppelklick/Enter öffenbar.
...
- Alle Doku-Symbole (eingebaut wie eigene, nicht mehr nur "Abwesend") lassen sich jetzt auch direkt im Sitzplan-Raster per Tastenkürzel togglen — im Raster wirkt das immer auf das heutige Datum (dort gibt es keine Datumsauswahl). In der Dokuansicht wirkt jedes Kürzel wie gewohnt auf die dort gerade ausgewählte Datumsspalte, auch die Leertaste (vorher schaltete die Leertaste dort immer das heutige Datum um, unabhängig vom angezeigten Termin — jetzt konsistent zu allen anderen Kürzeln). Die Symbol-Verwaltung und der "Symbol setzen"-Dialog zeigen für "Abwesend" jetzt "Leertaste" statt "u" als Kürzel an. Die Statuszeilen-Bestätigung ("heute anwesend/abwesend") beim Umschalten im Raster entfällt dabei ersatzlos, analog zum Toggle-Verhalten von Diagnosesymbolen im Raster, das ebenfalls ohne Bestätigungstext auskommt.
...
- Beim Wechsel in die Dokumentationsansicht war immer das älteste statt das neueste Datum ausgewählt.
