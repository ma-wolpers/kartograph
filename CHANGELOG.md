...
### Added
- Neue "Symbol-Verwaltung" (Ansicht-Menü und Editor-Toolbar): zeigt alle Symbole — eingebaute mit ihren Bewertungsstufen-Erklärungen und eigene, pro Sitzplan definierte Doku-Symbole — in einer Liste. Über "Neu" lassen sich eigene Doku-Symbole anlegen (ein beliebiges Zeichen/Emoji einfügen, eine Bedeutung eintragen, ein Tastenkürzel der Form `Ctrl+Shift+<Buchstabe>` vergeben, z. B. `Ctrl+Shift+K`), über "Bearbeiten"/"Löschen" wieder ändern bzw. entfernen (Symbol/Kürzel sind änderbar, die Bedeutung nicht — für eine Umbenennung: löschen und neu anlegen). Eigene Symbole lassen sich genau wie "Abwesend"/"Nicht abgegeben" tagesbezogen in der Dokutabelle setzen (per Maus oder dem vergebenen Tastenkürzel) und erscheinen dadurch am aktuellen Tag auch temporär auf dem Tisch im Sitzplan-Raster. Wird ein eigenes Symbol gelöscht, bleiben bereits erfasste Dokumentationstage erhalten (sie zeigen es danach als "Gelöschtes Symbol" an).
